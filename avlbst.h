...
void AVLTree<Key, Value>::insert (const std::pair<const Key, Value> &new_item)
{
    ...
    if(AVLRoot == nullptr) //if empty tree
    {
      //insert keyValuePair
      ...
      newPair->setBalance(0); //set balance to 0
    ...
    while (current) //parent exists, is not null
    {
      parent=current;
      //go to leaf node
      ...
      else if (new_item.first == current->getKey()) //same key
      {
    ...
    //if parent is -1 or +1, parent is now 0;
...
//root isn't avl notde, must cast to avl node
template<class Key, class Value>
void AVLTree<Key, Value>::insertFix(AVLNode<Key, Value>* parent, AVLNode<Key, Value>* current)
{
    ...
    else if(grandparent->getLeft() != nullptr && grandparent->getLeft() ==parent)
    {
      ...
      {
        if(parent->getLeft() !=nullptr && parent->getLeft() == current) //zig zig
        {
            rotateRight(grandparent);
            parent->setBalance(0);
            grandparent->setBalance(0);
        }
        else if(parent->getRight() !=nullptr && parent->getRight() ==current) //zig zag
        {
          rotateLeft(parent);
          rotateRight(grandparent);
    ...
    else if(grandparent->getRight() != nullptr && grandparent->getRight() ==parent)
    {
      ...
      {
        if(parent->getRight() !=nullptr && parent->getRight() == current) //zig zig
        {
            rotateLeft(grandparent);
            parent->setBalance(0);
            grandparent->setBalance(0);
        }
        else if(parent->getLeft() !=nullptr && parent->getLeft() ==current) //zig zag
        {
          rotateRight(parent);
          rotateLeft(grandparent);
          if(current->getBalance() == 1) //did not change the if statements...
          {
              parent->setBalance(0);
              grandparent->setBalance(-1);
              current->setBalance(0);
...
void AVLTree<Key, Value>:: remove(const Key& key)
{
    ...
    if(current==AVLRoot) //is root, no parent
    {
      ...
      else if (current->getLeft() != nullptr && current->getRight() != nullptr) //has 2 children
      {
    ...
    else if(current->getParent() != nullptr) //has parent
    {
      ...
      if(currParent->getLeft() != nullptr && currParent->getLeft() == current)
      {
        ...
        else if (current->getLeft() != nullptr && current->getRight() != nullptr) //2 children
        {
          nodeSwap(current, currPred);
          currParent=current->getParent();
          if(current->getLeft() == nullptr) //node has no left child
          {
            if(current->getParent()->getLeft() != nullptr && 
              current->getParent()->getLeft() ==current)
            {
          ...
          else if(current->getLeft() != nullptr) //node has left child
          {
            current->getLeft()->setParent(current->getParent());
            if(current->getParent()->getLeft() != nullptr && 
              current->getParent()->getLeft() == current)
      ...
      if(currParent->getRight() != nullptr && currParent->getRight() == current) //is right child
      {
        diff=-1;
        if(current->getLeft() == nullptr && current->getRight() == nullptr) //no children
        {
...
void AVLTree<Key, Value>::removeFix(AVLNode<Key, Value>* current, int diff)
{
  ...
  //store getBalance into a variable
...
void AVLTree<Key, Value>::rotateLeft(AVLNode<Key, Value>* parent)
{
  ...
  //if parent is on the left side of grandparent, then you set the left child of grandparent to be current
